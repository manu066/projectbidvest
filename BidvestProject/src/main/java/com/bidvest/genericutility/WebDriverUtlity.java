...
public class WebDriverUtlity {
	...
	public void waitForElementInDOM(WebDriver driver) {
		driver.manage().timeouts().implicitlyWait(IConstatnts.Implicitly_TIMEOUT, TimeUnit.SECONDS);
	}
	...
	public void waitForPage(WebDriver driver , String partailPageURL) {
		WebDriverWait wait = new WebDriverWait(driver, IConstatnts.Explicitly_TIMEOUT);
	 ...
	 */
	 ...
	 public String swithToAlertWindowAndAccpectvalidate(WebDriver driver )
	 { 
		 Alert alt = driver.switchTo().alert(); 
		 String actualResult=alt.getText().trim();
			 alt.accept(); 
			 return actualResult; 
	}
	...
	public List<WebElement> select(WebElement element)
	{
		Select sel=new Select(element);
